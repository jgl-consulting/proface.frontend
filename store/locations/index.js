...
export const actions = {
    async fetchLocations({ state, commit }, pagination) {
        const { requestPage, size, sortBy, descending } = pagination || state.pagination;
        const direction = descending ? 'desc' : 'asc';
        const { locations, page } = await this.$locations.pageLocations(requestPage, size, sortBy, direction);
    ...
    async deleteLocation({ dispatch }, { location }) {
