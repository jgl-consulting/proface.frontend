...
export const actions = {
    async fetchBanks({ state, commit }, pagination) {
        const { requestPage, size, sortBy, descending } = pagination || state.pagination;
        const direction = descending ? 'desc' : 'asc';
        const { banks, page } = await this.$banks.pageBanks(requestPage, size, sortBy, direction);
    ...
    async createBank({ dispatch }, { bank }) {
        await this.$banks.createBank(bank);
        await dispatch('fetchBanks');
    },
    async updateBank({ dispatch }, { bank }) {
        await this.$banks.updateBank(bank);
        await dispatch('fetchBanks');
    },
    async deleteBank({ dispatch }, { bank }) {
