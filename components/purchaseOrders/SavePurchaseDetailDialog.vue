...
export default {
  ...
  watch: {
    purchaseDetail: {
      handler() {
        this.purchaseDetailModel = JSON.parse(JSON.stringify(this.purchaseDetail));
        /*if(!this.purchaseDetailModel.status || this.purchaseDetailModel.status.id === 0){ 
          this.purchaseDetailModel.status = this.statuses || [{ id: 0 }];
        }*/
      }
    }
  },
  computed: {
    ...
    quantityRules() {
      return [
        (value) => required(value, 'La cantidad es requerido')
      ]
    }
  },
  methods: {
    ...
    async savePurchaseDetail() {
      ...
      try {
        ...
        const res = await this.$confirm(`¿Está seguro de guardar el detalle del producto '${product.name}'?`, { title: 'Advertencia' })
        if(res) {
          if(this.mode === 'nuevo') {
